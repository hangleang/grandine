...
use fork_choice_store::{
    ...
    DataColumnSidecarAction, DataColumnSidecarOrigin, PayloadAction, StateCacheProcessor, Store,
    ValidAttestation,
};
    ...
    VS: UnboundedSink<ValidatorMessage<P, W>>,
{
    ...
    fn handle_tick(&mut self, wait_group: &W, tick: Tick) -> Result<()> {
        ...
        if self.store.is_forward_synced() && misc::slots_since_epoch_start::<P>(tick.slot) == 0 {
            if tick.kind == TickKind::AttestFourth {
                ...
                self.prune_old_data_column_sidecars()?;
            }
    ...
    fn handle_store_sampling_columns(&mut self, sampling_columns: HashSet<ColumnIndex>) {
        let mut sorted_items = sampling_columns.into_iter().collect::<Vec<_>>();
        ...
        self.store_mut().store_sampling_columns(sorted_items.into());
    }
    ...
    fn prune_old_blob_sidecars(&self) -> Result<()> {
        ...
        Builder::new()
            ...
            .spawn(move || {
                debug!("pruning old blob sidecars from storage up to slot {up_to_slot}…");

                match storage.prune_old_blob_sidecars(up_to_slot) {
                    Ok(()) => {
                        debug!("pruned old blob sidecars from storage up to slot {up_to_slot}");
                    }
                    Err(error) => {
    ...
    fn prune_old_data_column_sidecars(&self) -> Result<()> {
        let storage = self.storage.clone_arc();
        let current_epoch = misc::compute_epoch_at_slot::<P>(self.store.slot());
        let up_to_epoch = current_epoch.saturating_sub(
            self.store
        ...
        Builder::new()
            .name("old-data-column-pruner".to_owned())
            .spawn(move || {
                debug!("pruning old data column sidecars from storage up to slot {up_to_slot}…");

                match storage.prune_old_data_column_sidecars(up_to_slot) {
                    Ok(()) => {
                        debug!(
                            "pruned old data column sidecars from storage up to slot {up_to_slot}"
                        );
                    }
                    Err(error) => {
