...
mod spec_tests {
    ...
    processing_tests! {
        ...
        |config, state, consolidation_request, _| electra::process_consolidation_request(config, state, consolidation_request),
    ...
    processing_tests! {
        ...
        |config, state, deposit, _| {
            ...
            electra::process_deposit_data(config, state, deposit.data)?;
            Ok(())
        },
        "deposit",
        "consensus-spec-tests/tests/mainnet/fulu/operations/deposit/*/*",
    ...
    processing_tests! {
        ...
        |_, state, deposit_request, _| electra::process_deposit_request(state, deposit_request),
        "deposit_request",
    ...
    processing_tests! {
        ...
        |config, state, withdrawal_request, _| electra::process_withdrawal_request(config, state, withdrawal_request),
        "withdrawal_request",
    ...
    fn run_processing_case<P: Preset, O: SszReadDefault>(
        case: Case,
        operation_name: &str,
        processing_function: impl FnOnce(&Config, &mut FuluBeaconState<P>, O, BlsSetting) -> Result<()>,
    ) {
    ...
    fn run_validation_case<P: Preset, O: SszReadDefault, R: Debug>(
        case: Case,
        operation_name: &str,
        validation_function: impl FnOnce(&Config, &mut FuluBeaconState<P>, O) -> Result<R>,
    ) {
        let mut state = case.ssz_default("pre");
        let operation = case.ssz_default(operation_name);
