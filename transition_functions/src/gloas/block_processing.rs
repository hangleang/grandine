...
use helper_functions::{
    accessors::{
        ...
        get_indexed_payload_attestation, initialize_shuffled_indices,
    },
    electra::{
        get_attesting_indices, is_fully_withdrawable_validator, is_partially_withdrawable_validator,
    },
    error::SignatureKind,
    gloas::slash_validator,
    misc::{self, get_max_effective_balance},
...
use types::{
    ...
    nonstandard::{AttestationEpoch, RelativeEpoch, SlashingKind},
    phase0::{
    ...
    preset::Preset,
    traits::{AttesterSlashing, PostGloasBeaconBlockBody, PostGloasBeaconState},
};
...
pub fn process_execution_payload_bid<P: Preset>(
    ...
    *state
        ...
        .mod_index_mut(misc::builder_payment_index_for_current_epoch::<P>(slot)) = pending_payment;
...
pub fn process_operations<P: Preset, V: Verifier>(
    ...
    // TODO(gloas): change to `electra::process_attester_slashing` once it's gloas compatible
...
pub fn apply_attestation<P: Preset>(
    ...
    let mut payment = match attestation_epoch {
        AttestationEpoch::Previous => state.builder_pending_payments().get(
            misc::builder_payment_index_for_previous_epoch::<P>(attestation.data.slot),
        ),
        AttestationEpoch::Current => state.builder_pending_payments().get(
            misc::builder_payment_index_for_current_epoch::<P>(attestation.data.slot),
        ),
    }?
    .to_owned();
    ...
    for (validator_index, base_reward, effective_balance) in attesting_indices_with_base_rewards {
        ...
        *epoch_participation |= participation_flags;
        ...
        if will_set_new_flag && is_attestation_same_slot {
...
pub fn process_proposer_slashing<P: Preset>(
    ...
    let relative_epoch = accessors::relative_epoch(state, misc::compute_epoch_at_slot::<P>(slot))?;
    match relative_epoch {
        RelativeEpoch::Current => {
            *state
                .builder_pending_payments_mut()
                .mod_index_mut(misc::builder_payment_index_for_current_epoch::<P>(slot)) =
                BuilderPendingPayment::default();
        }
        RelativeEpoch::Previous => {
            *state
                .builder_pending_payments_mut()
                .mod_index_mut(misc::builder_payment_index_for_previous_epoch::<P>(slot)) =
                BuilderPendingPayment::default();
        }
        RelativeEpoch::Next => {}
    }
