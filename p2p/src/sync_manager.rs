...
impl SyncManager {
    ...
    pub fn build_forward_sync_batches<P: Preset>(
        ...
        let mut start_slot = sync_start_slot;
        let mut max_slot = local_head_slot;

        let mut sync_batches = vec![];
        for peer_id in Self::peer_sync_batch_assignments(&peers_to_sync).take(batches_in_front) {
            // Enforce sync batch within an epoch at the fork boundary
            let start_slot_phase = config.phase_at_slot::<P>(start_slot);
            let end_slot_phase = config.phase_at_slot::<P>(start_slot + slots_per_request);
            let count = if start_slot_phase == end_slot_phase {
                slots_per_request.min(remote_head_slot.saturating_sub(start_slot) + 1)
            } else {
                let fork_epoch = config.fork_epoch(end_slot_phase);
                misc::compute_start_slot_at_epoch::<P>(fork_epoch).saturating_sub(start_slot)
            };
