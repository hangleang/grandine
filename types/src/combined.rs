...
impl<P: Preset> BeaconState<P> {
    pub fn with_execution_payload_header(
        ...
        match (&mut self, execution_payload_header) {
            ...
            (Self::Fulu(state), ExecutionPayloadHeader::Deneb(header)) => {
                state.latest_execution_payload_header = header;
            }
            (_, header) => {
...
impl<P: Preset> BeaconBlock<P> {
    ...
    pub fn with_execution_payload(
        ...
        match (&mut self, execution_payload) {
            ...
            (Self::Fulu(block), ExecutionPayload::Deneb(payload)) => {
                block.body.execution_payload = payload;
            }
            (_, payload) => {
                // This match arm will silently match any new phases.
    ...
    pub fn with_blob_kzg_commitments(
        ...
        match &mut self {
            ...
            Self::Fulu(block) => block.body.blob_kzg_commitments = commitments,
            _ => {
                // This match arm will silently match any new phases.
                // Cause a compilation error if a new phase is added.
                const_assert_eq!(Phase::CARDINALITY, 8);
    ...
    pub fn with_execution_requests(
        ...
        match &mut self {
            ...
            Self::Fulu(block) => block.body.execution_requests = execution_requests,
            _ => {
                // This match arm will silently match any new phases.
                // Cause a compilation error if a new phase is added.
                const_assert_eq!(Phase::CARDINALITY, 8);
...
impl<P: Preset> From<BeaconBlock<P>> for SignedBeaconBlock<P> {
    fn from(beacon_block: BeaconBlock<P>) -> Self {
        match beacon_block {
            ...
            .into(),
            BeaconBlock::Fulu(message) => FuluSignedBeaconBlock {
                message,
                signature: SignatureBytes::default(),
            }
            .into(),
...
pub enum SignedBlindedBeaconBlock<P: Preset> {
    ...
    Fulu(FuluSignedBlindedBeaconBlock<P>),
}
...
impl<P: Preset> SszSize for SignedBlindedBeaconBlock<P> {
    ...
    const SIZE: Size = Size::for_untagged_union::<{ Phase::CARDINALITY - 3 }>([
        ...
        FuluSignedBlindedBeaconBlock::<P>::SIZE,
    ]);
}
...
impl<P: Preset> SszWrite for SignedBlindedBeaconBlock<P> {
    fn write_variable(&self, bytes: &mut Vec<u8>) -> Result<(), WriteError> {
        match self {
            ...
            Self::Fulu(signed_blinded_block) => signed_blinded_block.write_variable(bytes),
        }
    }
}
...
impl<P: Preset> TryFrom<SignedBeaconBlock<P>> for SignedBlindedBeaconBlock<P> {
    ...
    fn try_from(signed_block: SignedBeaconBlock<P>) -> Result<Self, Self::Error> {
        ...
        let block = match beacon_block.try_into()? {
            ...
            BlindedBeaconBlock::Fulu(message) => {
                FuluSignedBlindedBeaconBlock { message, signature }.into()
            }
        };
...
impl<P: Preset> SignedBlindedBeaconBlock<P> {
    ...
    pub fn execution_payload_header(&self) -> &dyn ExecutionPayloadTrait<P> {
        match self {
            ...
            Self::Fulu(block) => &block.message.body.execution_payload_header,
        }
    }
}
...
impl<P: Preset> BlindedBeaconBlock<P> {
    pub fn with_signature(self, signature: SignatureBytes) -> SignedBlindedBeaconBlock<P> {
        match self {
            ...
            Self::Fulu(message) => FuluSignedBlindedBeaconBlock { message, signature }.into(),
        }
    }
...
impl<P: Preset> SszRead<Config> for DataColumnSidecar<P> {
    fn from_ssz_unchecked(_config: &Config, bytes: &[u8]) -> Result<Self, ReadError> {
        match GloasDataColumnSidecar::from_ssz_default(bytes) {
            Ok(sidecar) => Ok(sidecar.into()),
            Err(_) => FuluDataColumnSidecar::from_ssz_default(bytes).map(Into::into),
        }
    }
}
