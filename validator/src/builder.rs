...
impl<P: Preset, W: Wait> Builder<P, W> {
        ...
        loop {
            select! {
                ...
                p2p_message = self.p2p_to_builder_rx.select_next_some() => {
                    match p2p_message {
                        P2pToBuilder::ProposerPreferences(proposer_preference, gossip_id) => {
                            self.handle_proposer_preference(*proposer_preference, gossip_id).await
                        }
                    }
                },
    ...
    async fn handle_tick(&mut self, _wait_group: W, tick: Tick) -> Result<()> {
        ...
        if tick.is_start_of_slot() && self.builder_config.always_bid {
            ...
            for own_signed_payload_bid in own_signed_payload_bids {
                let signed_payload_bid = Arc::new(*own_signed_payload_bid);

                self.controller
                    .on_own_execution_payload_bid(signed_payload_bid.clone_arc());

                BuilderToP2p::PublishPayloadBid(signed_payload_bid).send(&self.p2p_tx);
            }
    ...
    #[instrument(parent = None, level = "debug", fields(service = "builder"), skip_all)]
    async fn handle_proposer_preference(
        &self,
        ...
        let outcome = match self
            .block_producer
            .handle_external_proposer_preference(proposer_preference)
            .await
        {
            Ok(outcome) => outcome,
            Err(error) => {
                warn_with_peers!("failed to handle proposer preference: {error}");
                return;
            }
        };
    ...
    fn handle_pool_addition_outcome_for_p2p(
        ...
        let message = match outcome {
            PoolAdditionOutcome::Accept => BuilderToP2p::Accept(gossip_id),
            PoolAdditionOutcome::Ignore => BuilderToP2p::Ignore(gossip_id),
            PoolAdditionOutcome::Reject(rejection_error, _) => {
                BuilderToP2p::Reject(gossip_id, rejection_error)
            }
        };
